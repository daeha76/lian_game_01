...
const makeMacaronRecipes = (): Recipe[] => {
  ...
  return fruits.map<Recipe>(({ id, name, fruit, fruitEmoji, color }) => ({
    ...
    steps: [
      ...
      { type: "knead",     message: `${fruit}색 반죽을 동글동글 만져봐! 5번`, target: 5 },
      { type: "pipe",      message: "쟁반에 동그란 마카롱 6개를 짜봐! 짜주머니를 콕콕!", count: 6 },
      { type: "appliance_in",   message: "마카롱 쟁반을 에어프라이기에 끌어다 놓아봐!", appliance: "air_fryer" },
      ...
      { type: "take_out",  message: "구운 마카롱 6개를 꺼내자!" },
      { type: "macaron_sandwich", message: `${fruit} 크림 발라서 두개씩 짝지어 줘! 3개 만들어요`, pairs: 3, color },
      { type: "eat",       message: `친구에게 마카롱 3개를 하나씩 줘봐! 입에 끌어다 놓으면 냠냠`, bites: 3 },
      { type: "finale",    message: "완성! 정말 잘했어! 🎉", button: "다시 하기" },
    ],
  }));
};
...
const makeRollcakeRecipes = (): Recipe[] => {
  ...
  return fruits.map<Recipe>(({ id, name, fruit, fruitEmoji, color }) => ({
    ...
    steps: [
      ...
      { type: "whisk",     message: "이제 거품기로 빙글빙글 저어봐! 세 바퀴~", target: 3 },
      { type: "appliance_in",   message: "시트 반죽을 오븐에 끌어다 놓아봐!", appliance: "oven" },
      { type: "appliance_run",  message: "오븐에서 시트가 노릇노릇~ 🔥", duration: 3000, appliance: "oven" },
